...
def build_dictionary_markdown(reports: Reports) -> str:
    ...
    lines.append(
        "_Generated by `python -m app.data_dictionary` from the Phase 3 reports. "
    ...
    if reports.columns.empty:
        lines.append(
            "> **No column report found.** Run `python -m app.schema_discovery` " "first.\n"
        )
        return "\n".join(lines)
